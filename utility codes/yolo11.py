#approach on segregating mae part and then twice ocr
...
def remove_noise_and_smooth(img):
	...
	#filtered = cv2.adaptiveThreshold(img_grey.astype(np.uint8), 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
	blur = cv2.GaussianBlur(img_grey,(5,5),0)
	#ret3,th3 = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
	median = cv2.medianBlur(blur,5)
	#median = cv2.bitwise_not(median)
	#kernel = cv2.getStructuringElement(cv2.MORPH_DILATE, (5, 2))
...
#imgg_path = "D:/fortiate/IMG_20210415_163829.jpg"
#imgg_path = "D:/fortiate/a-copy.jpg"
imgg_path = "D:/fortiate/project/sample_photos/pancard_type1.jpg"
...
def object_detector(images_path):
	...
	if pan2 == 1:
		...
		gray = remove_noise_and_smooth(img_plate)
		xxx1 = width*0.0382
		xxx2 = width*0.347
		yyy1 = height*0.5525
		yyy2 = height*0.6137
	...
	if aadhar_flag == 1:
		...
		#cv2.imshow('img',cropped)
		#cv2.waitKey(0)
		th, threshed = cv2.threshold(cropped, 127, 255, cv2.THRESH_BINARY)
		hImg, wImg,_ = img.shape
		cv2.imshow("imggg", cropped)
		cv2.imshow("imggg", threshed)
		cv2.waitKey(0)
		
		kernel = np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]])
		im = cv2.filter2D(cropped, -1, kernel)
		cv2.imwrite("hih.jpg" , im)


		## (3) Detect
		result = pytesseract.image_to_string((im), config = r'-l eng+hin --psm 6')
		detect_langs(result)
		#print(result)
		## (5) Normalize
		for word in result.split("\n"):
		...
		if len(search_result_dob) == 0:
			...
			dob_year = re.findall(dob_year_pattern, result)
			print(dob_year)
		...
		result_dict["gender"] = gender_pattern1

		gender_2 = r'\bFemale\b'
		gender_pattern2 = re.match(gender_2, result)
		result_dict["gender"] = gender_pattern2
		'''

		search = "MALE"
		gender_pattern1 = re.findall(r"\b" + search + r"\b", result)
		search = "FEMALE"
		gender_pattern2 = re.findall(r"\b" + search + r"\b", result)


		if gender_pattern1 == None:
			result_dict["gender"] = gender_pattern2
		...
		print(gender_pattern1)
		#print(gender_pattern2)
		print(result)
		...
		#print(result)
		#exit()
		#result_dict["name"] = name_pattern
		#result_dict["dob"] = str(search_result_dob)
		result_dict["id_number"] = search_result_aadhar_number_pattern 
