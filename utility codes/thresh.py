...
if __name__ == '__main__':
    ratio=1
    #image = cv2.imdecode(np.fromfile('IMG_20210415_163829.jpg', dtype=np.uint8), 0)
    imggggg = cv2.imread('riyasat.jpg',cv2.IMREAD_COLOR).dtype
    print(imggggg)
    ...
    threshh = cv2.adaptiveThreshold(img,  255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 9, 9)
    # retval, thresh = cv2.threshold(img, 150, 255, cv2.THRESH_OTSU)
    # retval, thresh = cv2.threshold(img, retval, 255, cv2.THRESH_OTSU)
    cv2.imwrite('adaptivethresh_results.jpg',threshh)
    '''
    ...
    #cv2.imshow("", thresh.astype('uint8') * 255)
    cv2.imwrite('results.jpg', np.uint8(thresh))
    #imagem = cv2.bitwise_not(thresh)
    thresh = 255 - thresh
    cv2.imwrite('results_grayconv.jpg', np.uint8(thresh))
