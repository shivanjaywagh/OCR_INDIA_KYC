...
def intergral_thresh(imgg):
	...
	for i in range(nRows):
		...
		# else:
		#outputMat[j][i] = 0
	new_p = Image.fromarray(outputMat)
	if new_p.mode != 'RGB':
...
def object_detector(images_path):
	...
	threshedint = intergral_thresh(img_bw)
	...
	if pan2 == 1:
		...
		result = pytesseract.image_to_string((threshedint), config = r'-l eng+hin --psm 6')
